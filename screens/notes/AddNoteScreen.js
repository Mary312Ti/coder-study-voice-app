...
import WarningModal from "../../components/notes/warningModal.js";
...
export default function AddNoteScreen({ navigation }) {
    ...
    db.transaction((tx) => {
      selectedTags.forEach((tag) => {
        tx.executeSql(
          ...
          [noteId, tag],
          (_, { insertId }) =>
            console.log(`Tag ${tag} attached to note ${noteId}`),
  ...
  return (
    <View style={{ flex: 1, padding: 16 }}>
      ...
      <WarningModal isVisible={showWarning} onClose={handleCloseWarning} />
    </View>
  );
}
